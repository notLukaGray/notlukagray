...
export function SvgCustomJsonPanel({ controller }: { controller: SvgElementDevController }) {
  ...
  const applyJson = () => {
    ...
    const next = controller.normalizeVariant(
      controller.active,
      parsed.value as Partial<SvgVariantDefaults>
    );
  ...
  return (
    <section className="space-y-3 rounded-lg border border-border bg-card/20 p-4">
      ...
      <textarea
        ...
        placeholder="Paste SVG variant JSON object here…"
      />
