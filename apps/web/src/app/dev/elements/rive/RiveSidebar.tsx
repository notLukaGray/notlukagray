...
function HandoffSnippetPanel({ controller }: { controller: RiveElementDevController }) {
  return (
    <div className="space-y-3 rounded-lg border border-border bg-card/20 p-4">
      ...
      <p className="text-[10px] leading-snug text-muted-foreground">
        Share this JSON with engineering to wire these Rive variants into runtime defaults.
      </p>
      ...
      {!controller.hydrated ? (
        <p className="text-[10px] text-muted-foreground">Loading saved Rive defaults…</p>
      ) : null}
    </div>
  );
}
