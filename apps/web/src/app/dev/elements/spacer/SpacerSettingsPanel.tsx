...
import { SpacerAnimationControls } from "./controls/SpacerAnimationControls";
import { SpacerContentControls } from "./controls/SpacerContentControls";
import { SpacerLayoutControls } from "./controls/SpacerLayoutControls";
import { SpacerRuntimeControls } from "./controls/SpacerRuntimeControls";
import { SpacerTraitsControls } from "./controls/SpacerTraitsControls";
import type { SpacerElementDevController } from "./useSpacerElementDevController";

function getSettingsLabel(controller: SpacerElementDevController): string {
  return controller.isCustomVariant ? "Create Custom" : VARIANT_LABELS[controller.activeVariant];
}

export function SpacerSettingsPanel({ controller }: { controller: SpacerElementDevController }) {
      ...
      {hasAnyCategoryVisible ? (
        <div className="grid gap-4 sm:grid-cols-2">
          {controller.visibleCategories.content ? (
            <SpacerContentControls controller={controller} />
          ) : null}
          {controller.visibleCategories.layout ? (
            <SpacerLayoutControls controller={controller} />
          ) : null}
          {controller.visibleCategories.traits ? (
            <SpacerTraitsControls controller={controller} />
          ) : null}
          {controller.visibleCategories.animation ? (
            <SpacerAnimationControls controller={controller} />
          ) : null}
          {controller.visibleCategories.runtime ? (
            <SpacerRuntimeControls controller={controller} />
          ) : null}
        </div>
      ) : (
