...
export function Model3dCustomJsonPanel({
  controller,
}: {
  ...
  const applyJson = () => {
    ...
    const next = controller.normalizeVariant(
      controller.active,
      parsed.value as Partial<Model3dVariantDefaults>
    );
  ...
  return (
    <section className="space-y-3 rounded-lg border border-border bg-card/20 p-4">
      ...
      <p className="text-[10px] text-muted-foreground">
        Custom JSON edits only the Create Custom draft. Built-in variants remain unchanged. Paste a
        full <code className="font-mono text-[10px]">elementModel3D</code> block here including
        scene, models, and camera.
      </p>
      <textarea
        value={text}
        ...
        placeholder="Paste model-3d variant JSON object here…"
      />
