...
export function VectorCustomJsonPanel({ controller }: { controller: VectorElementDevController }) {
  ...
  const applyJson = () => {
    ...
    const next = controller.normalizeVariant(
      controller.active,
      parsed.value as Partial<VectorVariantDefaults>
    );
  ...
  return (
    <section className="space-y-3 rounded-lg border border-border bg-card/20 p-4">
      ...
      <textarea
        ...
        placeholder="Paste vector variant JSON object here…"
      />
