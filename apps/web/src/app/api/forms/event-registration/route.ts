...
export async function POST(request: NextRequest) {
  const rateLimitRes = checkFormRateLimit(request, "event-registration");
  ...
  if (!to || typeof to !== "string") {
    return formErrorResponse("Event registration is not configured.", 503);
  }
  ...
  if (!ok) return formErrorResponse(error ?? "Registration failed. Try again.", 502);
  ...
  const cookie = buildFormRateLimitCookie(request, "event-registration");
