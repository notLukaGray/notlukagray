...
export function VideoCustomJsonPanel({ controller }: { controller: VideoElementDevController }) {
  ...
  const applyJson = () => {
    ...
    const next = controller.normalizeVariant(
      controller.active,
      parsed.value as Partial<VideoVariantDefaults>
    );
  ...
  return (
    <section className="space-y-3 rounded-lg border border-border bg-card/20 p-4">
      ...
      <textarea
        ...
        placeholder="Paste video variant JSON object here…"
      />
