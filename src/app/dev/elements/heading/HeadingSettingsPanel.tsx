...
import { HeadingAnimationControls } from "./controls/HeadingAnimationControls";
import { HeadingContentControls } from "./controls/HeadingContentControls";
import { HeadingLayoutControls } from "./controls/HeadingLayoutControls";
import { HeadingRuntimeControls } from "./controls/HeadingRuntimeControls";
      ...
      {hasAnyCategoryVisible ? (
        <div className="grid gap-4 sm:grid-cols-2">
          {controller.visibleCategories.content ? (
            <HeadingContentControls controller={controller} />
          ) : null}
          {controller.visibleCategories.layout ? (
            <HeadingLayoutControls controller={controller} />
          ) : null}
          {controller.visibleCategories.traits ? (
            <HeadingTraitsControls controller={controller} />
          ) : null}
          {controller.visibleCategories.animation ? (
            <HeadingAnimationControls controller={controller} />
          ) : null}
          {controller.visibleCategories.runtime ? (
            <HeadingRuntimeControls controller={controller} />
          ) : null}
        </div>
      ) : (
