...
export function InputCustomJsonPanel({ controller }: { controller: InputElementDevController }) {
  ...
  const applyJson = () => {
    ...
    const next = controller.normalizeVariant(
      controller.active,
      parsed.value as Partial<InputVariantDefaults>
    );
  ...
  return (
    <section className="space-y-3 rounded-lg border border-border bg-card/20 p-4">
      ...
      <textarea
        ...
        placeholder="Paste input variant JSON object here…"
      />
