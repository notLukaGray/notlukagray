...
export function useBodyElementDevController() {
  const [defaultVariant, setDefaultVariant] = useState<BodyVariantKey>(
    BASE_DEFAULTS.defaultVariant
  );
  const [variants, setVariants] = useState<Record<BodyVariantKey, BodyVariantDefaults>>(
    BASE_DEFAULTS.variants
  );
  const [activeVariant, setActiveVariant] = useState<BodyVariantKey>(BASE_DEFAULTS.defaultVariant);
  ...
  useEffect(() => {
    if (hydrated) {
      const payload: PersistedBodyDefaults = { v: 1, defaultVariant, variants };
      localStorage.setItem(STORAGE_KEY, JSON.stringify(payload));
    }
  }, [defaultVariant, hydrated, variants]);

  const updateVariant = useCallback(
    (key: BodyVariantKey, apply: (v: BodyVariantDefaults) => BodyVariantDefaults) => {
  ...
  const setVariantPatch = useCallback(
    (key: BodyVariantKey, patch: Partial<BodyVariantDefaults>) => {
  ...
  const setVariantExact = useCallback(
    (key: BodyVariantKey, next: BodyVariantDefaults) => {
  ...
  const setAnimationPatch = useCallback(
    (key: BodyVariantKey, patch: Partial<PbImageAnimationDefaults>) => {
  ...
  const patchEntranceFineTune = useCallback(
    (key: BodyVariantKey, patch: Partial<PbImageEntranceFineTune>) => {
      updateVariant(key, (v) => ({
        ...v,
        animation: {
          ...v.animation,
  ...
  const patchExitFineTune = useCallback(
    (key: BodyVariantKey, patch: Partial<PbImageExitFineTune>) => {
      updateVariant(key, (v) => ({
        ...v,
        animation: {
          ...v.animation,
  ...
  const setEntranceCurvePreset = useCallback(
    (key: BodyVariantKey, preset: PbImageAnimationCurvePreset) =>
  ...
  const setEntranceBezierValue = useCallback(
    (key: BodyVariantKey, index: number, value: number) =>
      patchEntranceFineTune(key, {
        curve: {
  ...
  const exportJson = useMemo(
    () => toBodyExportJson({ v: 1, defaultVariant, variants }),
    [defaultVariant, variants]
  );
  const customElementJson = useMemo(
    () => JSON.stringify(buildBodyElementSnippet(activeVariant, active, runtimeDraft), null, 2),
  ...
  const resetBodyDefaults = useCallback(() => {
  ...
  const selectVariant = useCallback((key: BodyVariantKey) => {
