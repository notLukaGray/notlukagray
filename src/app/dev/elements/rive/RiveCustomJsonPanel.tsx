...
import type { RiveVariantDefaults } from "./types";
import type { RiveElementDevController } from "./useRiveElementDevController";

export function RiveCustomJsonPanel({ controller }: { controller: RiveElementDevController }) {
  ...
  const applyJson = () => {
    ...
    const next = controller.normalizeVariant(
      controller.active,
      parsed.value as Partial<RiveVariantDefaults>
    );
  ...
  return (
    <section className="space-y-3 rounded-lg border border-border bg-card/20 p-4">
      ...
      <p className="text-[10px] text-muted-foreground">
        Custom JSON edits only the Create Custom draft. Built-in variants remain unchanged. Include{" "}
        <code className="font-mono text-[10px]">src</code>,{" "}
        <code className="font-mono text-[10px]">artboard</code>, and{" "}
        <code className="font-mono text-[10px]">stateMachine</code> for a complete element block.
      </p>
      <textarea
        value={text}
        ...
        placeholder="Paste Rive variant JSON object here…"
      />
