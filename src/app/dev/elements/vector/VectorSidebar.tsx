import { VectorCustomJsonPanel } from "./VectorCustomJsonPanel";
import type { VectorElementDevController } from "./useVectorElementDevController";

export function VectorSidebar({ controller }: { controller: VectorElementDevController }) {
  return (
    <div className="space-y-6 md:sticky md:top-8">
      {controller.isCustomVariant ? (
        <VectorCustomJsonPanel controller={controller} />
      ) : (
...
function HandoffSnippetPanel({ controller }: { controller: VectorElementDevController }) {
  return (
    <div className="space-y-3 rounded-lg border border-border bg-card/20 p-4">
      ...
      <p className="text-[10px] leading-snug text-muted-foreground">
        Share this JSON with engineering to wire these vector variants into runtime defaults.
      </p>
      ...
      {!controller.hydrated ? (
        <p className="text-[10px] text-muted-foreground">Loading saved vector defaults…</p>
      ) : null}
    </div>
  );
}
