import { LinkCustomJsonPanel } from "./LinkCustomJsonPanel";
import type { LinkElementDevController } from "./useLinkElementDevController";

export function LinkSidebar({ controller }: { controller: LinkElementDevController }) {
  return (
    <div className="space-y-6 md:sticky md:top-8">
      {controller.isCustomVariant ? (
        <LinkCustomJsonPanel controller={controller} />
      ) : (
...
function HandoffSnippetPanel({ controller }: { controller: LinkElementDevController }) {
  return (
    <div className="space-y-3 rounded-lg border border-border bg-card/20 p-4">
      ...
      <p className="text-[10px] leading-snug text-muted-foreground">
        Share this JSON with engineering to wire these link variants into runtime defaults
        (`pbBuilderDefaultsV1.elements.link`).
      </p>
      ...
      {!controller.hydrated ? (
        <p className="text-[10px] text-muted-foreground">Loading saved link defaults…</p>
      ) : null}
    </div>
  );
}
