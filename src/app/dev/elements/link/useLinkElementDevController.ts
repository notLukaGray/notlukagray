...
export function useLinkElementDevController() {
  const [defaultVariant, setDefaultVariant] = useState<LinkVariantKey>(
    BASE_DEFAULTS.defaultVariant
  );
  const [variants, setVariants] = useState<Record<LinkVariantKey, LinkVariantDefaults>>(
    BASE_DEFAULTS.variants
  );
  const [activeVariant, setActiveVariant] = useState<LinkVariantKey>(BASE_DEFAULTS.defaultVariant);
  ...
  useEffect(() => {
    if (hydrated) {
      const payload: PersistedLinkDefaults = { v: 1, defaultVariant, variants };
      localStorage.setItem(STORAGE_KEY, JSON.stringify(payload));
    }
  }, [defaultVariant, hydrated, variants]);

  const updateVariant = useCallback(
    (key: LinkVariantKey, apply: (v: LinkVariantDefaults) => LinkVariantDefaults) => {
  ...
  const setVariantPatch = useCallback(
    (key: LinkVariantKey, patch: Partial<LinkVariantDefaults>) => {
  ...
  const setVariantExact = useCallback(
    (key: LinkVariantKey, next: LinkVariantDefaults) => {
  ...
  const setAnimationPatch = useCallback(
    (key: LinkVariantKey, patch: Partial<PbImageAnimationDefaults>) => {
  ...
  const patchEntranceFineTune = useCallback(
    (key: LinkVariantKey, patch: Partial<PbImageEntranceFineTune>) => {
      updateVariant(key, (v) => ({
        ...v,
        animation: {
          ...v.animation,
  ...
  const patchExitFineTune = useCallback(
    (key: LinkVariantKey, patch: Partial<PbImageExitFineTune>) => {
      updateVariant(key, (v) => ({
        ...v,
        animation: {
          ...v.animation,
  ...
  const setEntranceCurvePreset = useCallback(
    (key: LinkVariantKey, preset: PbImageAnimationCurvePreset) =>
  ...
  const setEntranceBezierValue = useCallback(
    (key: LinkVariantKey, index: number, value: number) =>
      patchEntranceFineTune(key, {
        curve: {
  ...
  const exportJson = useMemo(
    () => toLinkExportJson({ v: 1, defaultVariant, variants }),
    [defaultVariant, variants]
  );
  const customElementJson = useMemo(
    () => JSON.stringify(buildLinkElementSnippet(activeVariant, active, runtimeDraft), null, 2),
  ...
  const resetLinkDefaults = useCallback(() => {
  ...
  const selectVariant = useCallback((key: LinkVariantKey) => {
