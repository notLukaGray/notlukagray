...
export function LinkCustomJsonPanel({ controller }: { controller: LinkElementDevController }) {
  ...
  const applyJson = () => {
    ...
    const next = controller.normalizeVariant(
      controller.active,
      parsed.value as Partial<LinkVariantDefaults>
    );
  ...
  return (
    <section className="space-y-3 rounded-lg border border-border bg-card/20 p-4">
      ...
      <textarea
        ...
        placeholder="Paste link variant JSON object here…"
      />
