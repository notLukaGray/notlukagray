...
const ROW_META: Record<keyof PbContentGuidelines, { label: string; hint: string }> = {
  copyTextAlign: {
    ...
    hint: "Default when the block has no `textAlign` or `align` (see `applyPbDefaultTextAlign`).",
  },
  frameGapWhenUnset: {
...
const SECTIONS: {
...
}[] = [
  {
    title: "Frames",
    blurb:
      "`elementGroup` (and module slots) support `gap`, `rowGap`, `columnGap`, `flexDirection`, `alignItems`, `justifyContent`, `flexWrap`, `padding`, `borderRadius` in page-builder data. Values below apply only when a field is **not set** on that block.",
    keys: [
      "frameGapWhenUnset",
      "frameRowGapWhenUnset",
      "frameColumnGapWhenUnset",
  ...
  },
  {
    title: "Interactive",
    blurb:
      "Naked **spacing / radius** tokens only (--pb-button-*). Label typography comes from element `copyType` + `/dev/fonts`, not here.",
    keys: [
      "buttonLabelGap",
      "buttonNakedPadding",
      "buttonNakedPaddingY",
...
export function StyleDevClient() {
  ...
  const allUnlocked = PB_GUIDELINE_KEYS.every((k) => !locks[k]);
  ...
  return (
    <main className="min-h-screen overflow-x-auto bg-background text-foreground">
      <div className="mx-auto w-full min-w-[768px] max-w-[1520px] px-6 py-8 md:px-10 md:py-10 lg:px-14 lg:py-12">
        <DevWorkbenchNav onResetSection={resetStyleTool} onTotalReset={resetStyleTool} />
        <header className="mb-8 space-y-4">
          <div>
            <h1 className="typography-heading-2xl">Spacing / Style</h1>
            <p className="mt-2 max-w-2xl text-[11px] font-mono uppercase tracking-wide text-muted-foreground">
              Layer 3 · Seeds + locks (like colors)
            </p>
          </div>
          <p className="typography-body-2xl max-w-3xl text-muted-foreground">
            ...
            while you keep tuning the rest. Previews show how tokens apply when a block **does not
            set** the matching property (see Frames blurb). Element typography (e.g. button labels)
            is tuned in{" "}
            <Link href="/dev/fonts" className="font-mono text-[0.9em] underline underline-offset-2">
              /dev/fonts
            </Link>{" "}
            + block props. Export replaces{" "}
            <code className="font-mono text-[0.9em]">pb-content-guidelines-config.ts</code>{" "}
            (expansion stays in{" "}
            <code className="font-mono text-[0.9em]">pb-guidelines-expand.ts</code>).
          </p>
          {allUnlocked ? (
            <p className="max-w-3xl text-[11px] text-amber-700 dark:text-amber-400">
          ...
          <div className="space-y-6">
            <section className="space-y-4 rounded-lg border border-border bg-card/20 p-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <label className="space-y-1.5">
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    Drives copy text-align and frame align-items while those rows are unlocked.
                  </p>
                </label>
                <label className="space-y-1.5">
                  <span className="font-mono text-[10px] uppercase tracking-wide text-muted-foreground">
                  ...
                  />
                  <p className="text-[10px] text-muted-foreground">
                    One dial for paragraph gap, headings, lists, buttons (see multiples in proposed
                    column).
                  </p>
                </label>
              </div>
              <label className="flex cursor-pointer select-none items-center gap-2 font-mono text-[11px] text-foreground">
                ...
                />
                <span>
                  Propose default frame gap{" "}
                  <span className="text-[10px] font-normal text-muted-foreground">
                    (2× spacing base; off → no theme fallback for gap)
                  </span>
                </span>
              </label>
              <p className="text-[10px] leading-relaxed text-muted-foreground">
                Live proposal from seeds only (ignores locks):{" "}
                <code className="rounded bg-muted/50 px-1 font-mono text-[10px]">
                  alignment → copy / frames; spacingBase → rhythm; frame gap toggle as above
                </code>
              </p>
            </section>
            ...
            <div className="space-y-3 rounded-lg border border-border bg-card/20 p-4">
              ...
              <p className="text-[10px] leading-snug text-muted-foreground">
                ...
                <code className="font-mono text-[0.95em]">
                  src/app/theme/pb-content-guidelines-config.ts
                </code>{" "}
                entirely. New CSS variables require editing{" "}
                <code className="font-mono text-[0.95em]">pb-guidelines-expand.ts</code> in the
                repo.
              </p>
