...
On the Next.js site, **`loadPageBuilder`** merges **`src/content/modules/*.json`** into the page’s definitions map **before** expand (`{ ...moduleFiles, ...pageDefinitions }`). The page JSON **wins** on key collision.
...
- **`src/content/modules/<key>.json`** for reusable blocks with `type: "module"`, or
...
`page-builder-validation.ts` may attribute `definitions.<key>` errors to either a page file or `src/content/modules/<key>.json` when reporting paths.

## See also

- [Designer workflow / ZIP layout](./08-workflow.md)
- [Expand / ids](./12-page-builder-expand-parity.md)
- [Page-builder pipeline](../../../../docs/03-page-builder-pipeline.md)
