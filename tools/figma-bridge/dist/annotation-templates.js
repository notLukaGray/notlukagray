...
const ELEMENT_ANNOTATION_TEMPLATES = [
...
const SECTION_ANNOTATION_TEMPLATES = [
    template("section", "type", "sectionColumn", "Force section block type"),
    template("section", "type", "scrollContainer", "Force scroll container (usually inferred from overflow)"),
    template("section", "type", "formBlock", "Force form section (usually inferred from input instances)"),
    ...
    template("section", "visiblewhen", "videoPlaying:equals:true", "Conditional visibility (variable:operator:value)"),
    template("section", "scrollopacity", "0:1:0.2:1", "Scroll opacity inputStart:inputEnd:outputStart:outputEnd (colon) or four comma numbers"),
...
function scoreAnnotationTemplate(node, t) {
    ...
    if (isText) {
        if (t.key === "seo" || t.key === "style")
            score += 25;
        if (t.key === "contentkey" || t.key === "variablekey")
            score += 12;
        if (t.key.startsWith("on"))
            score += 4;
    }
    if (isFrameLike) {
        if (t.scope === "section" && t.key === "type")
            score += 20;
        if (t.scope === "section" && (t.key === "onvisible" || t.key === "oninvisible"))
            score += 10;
        if (t.scope === "section" && (t.key === "overflow" || t.key === "hidden"))
            score += 8;
        if (node.childCount >= 3 && t.key === "type" && t.example === "sectionColumn")
            score += 20;
        if (node.childCount >= 8 && (t.key === "scrollspeed" || t.key === "sticky"))
            score += 9;
    }
    if (isVectorLike) {
        if (t.key === "objectfit" || t.key === "objectposition")
            score += 14;
        if (t.key === "src")
            score += 10;
    }
    if (name.includes("video")) {
        if (t.key === "src" || t.key === "poster")
            score += 20;
        if (t.key === "autoplay" || t.key === "loop" || t.key === "muted")
            score += 16;
        if (t.key === "showplaybutton")
            score += 12;
    }
    if (name.includes("button") || name.includes("cta")) {
        if (t.key === "type" && t.example === "button")
            score += 20;
        if (t.key === "label" || t.key === "href" || t.key === "action")
            score += 16;
        if (t.key === "variant" || t.key === "size" || t.key === "disabled")
            score += 10;
        if (t.key === "onclick")
            score += 12;
    }
    if (name.includes("modal")) {
        if (t.key === "action" && t.example.startsWith("modalOpen:"))
            score += 16;
    }
...
export function getAnnotationTemplateTotal(nodeType) {
    return getAnnotationTemplatesForNode(nodeType).length;
}
export function getTopAnnotationTemplates(node, limit) {
 ...
 */
export function getAllAnnotationTemplates(scope) {
    if (scope === "element")
        return [...ELEMENT_ANNOTATION_TEMPLATES];
    if (scope === "section")
        return [...SECTION_ANNOTATION_TEMPLATES];
