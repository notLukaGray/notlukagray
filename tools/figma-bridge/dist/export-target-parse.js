...
export function parseExportTargetFromLayerName(rawName) {
...
export function getLayerPrefixDiagnostics(rawName) {
    const name = stripAnnotations(rawName || "untitled");
    ...
    if (!KNOWN_PREFIXES.has(prefix)) {
        diagnostics.push(`Unknown prefix "${prefix || "(empty)"}" — export defaults to page; use Page/, Section/, Module/, etc.`);
    }
    return diagnostics;
}
