...
function resolveContentDir(): string {
  const legacyContentDir = path.join(process.cwd(), "src/content");
  if (fs.existsSync(legacyContentDir)) return legacyContentDir;
  return path.join(process.cwd(), "apps/web/src/content");
}
